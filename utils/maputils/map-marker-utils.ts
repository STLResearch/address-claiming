...
export const createAirRightEstimateMarker = (map: mapboxgl.Map, airRightEstimate: any) => {
  const {
  ...
  const estAnnualProjectionLabel = getEstimateAmountLabel(Number(estAnnualProjection));
  const estAnnualProjectionIcon = getEstimateValueIcon("circleDollar");
  const estAnnualProjectionContainer = getEstimateValueContainer("annualProjection");
